...
describe('A.4 — T212 CSV parser', () => {
  it.fails('A.4.1 happy-path: 47 rows → 47 trades, 12 positions, 0 errors', async () => {
  ...
  it.fails('A.4.2 bad-headers → errors[0].kind === bad_headers, no trades', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.3 partial-bad: 45 trades + 2 errors with row numbers', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.4 empty CSV (header only) → 0 trades, 0 positions, 0 errors', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.5 mixed-currency: each trade preserves native currency', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.6 parsing is deterministic (pure): same fixture → same output twice', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.7 trade IDs are stable across two parses of the same fixture', async () => {
    const { parseT212Csv } = await import('../../t212.js')
    const csv = fixture('t212-sample.csv')
    const r1 = parseT212Csv(csv)
    const r2 = parseT212Csv(csv)
  ...
  it.fails('A.4.8 CRLF line endings parse identically to LF', async () => {
    const { parseT212Csv } = await import('../../t212.js')
  ...
  it.fails('A.4.9 BOM prefix parses identically', async () => {
    const { parseT212Csv } = await import('../../t212.js')
    const lf = fixture('t212-sample.csv')
    const bom = '\uFEFF' + lf
    const r1 = parseT212Csv(lf)
  ...
  it.fails('A.4.10 quoted fields with commas are preserved', async () => {
    const { parseT212Csv } = await import('../../t212.js')
    const csv = [
