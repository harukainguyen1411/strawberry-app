...
function makeFirestoreMock() {
  ...
  const buildCollection = (prefix: string) => ({
    ...
    get: vi.fn().mockResolvedValue({
      docs: [...store.entries()]
        .filter(([k]) => k.startsWith(prefix + '/') && k.split('/').length === prefix.split('/').length + 2)
  ...
  const batch = () => {
    ...
    return {
      set: (ref: { _key: string }, data: Record<string, unknown>, opts?: { merge?: boolean }) => {
        ops.push(() => {
          if (opts?.merge) {
            store.set(ref._key, { ...(store.get(ref._key) ?? {}), ...data })
          } else {
...
describe('V0.1.0 — importT212Pdf callable integration', () => {
  ...
  it.fails('PDF-INT-01 import T212 PDF fixture: 13 positions written, cash set, fxRates seeded', async () => {
  ...
  it.fails('PDF-INT-02 positions are full-replaced (old position removed)', async () => {
    const { importT212Pdf } = await import('../importT212Pdf.js')
  ...
  it.fails('PDF-INT-03 fxRates merged (not overwritten) — preserves existing pairs', async () => {
    const { importT212Pdf } = await import('../importT212Pdf.js')
  ...
  it.fails('PDF-INT-04 unauthenticated call throws with code unauthenticated', async () => {
    const { importT212Pdf } = await import('../importT212Pdf.js')
    const buf = fixture('t212-statement.pdf')
    await expect(
  ...
  it.fails('PDF-INT-05 non-PDF buffer returns error in result', async () => {
    const { importT212Pdf } = await import('../importT212Pdf.js')
  ...
  it.fails('PDF-INT-06 IB cash unaffected by T212 PDF import', async () => {
    const { importT212Pdf } = await import('../importT212Pdf.js')
